...
class GRPOFullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def generate_trajectory(
        ...
        rewards, successes, metadata = batched_rewards(self._tokenizer, responses, answers, device=self._device)
    ...
    def generate_trajectory_batched(
        ...
        for field_name in trajectories[0]._fields:
            if field_name == 'answers':
                ...
                # Concatenate tensors
                concatenated_fields[field_name] = torch.cat([getattr(traj, field_name) for traj in trajectories])
                ...
                if self._steps_run % self._log_every_n_steps == 0:
                    ...
                    for field_name in grpo_stats[0]._fields:
                        if field_name == 'metadata':
                            ...
                            # Stack tensors
                            concatenated_stats[field_name] = torch.stack([getattr(stat, field_name) for stat in grpo_stats])
